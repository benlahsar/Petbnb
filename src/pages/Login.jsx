            ...
            <div className="hidden lg:flex lg:w-1/2 bg-emerald-800 relative overflow-hidden">
                <div className="absolute inset-0 grid grid-cols-6 grid-rows-6 gap-4">
                    ...
                    {Array(36).fill().map((_, i) => {
                        ...
                        const shapes = [
                            <div key={i}
                                 className={`rounded-full bg-white opacity-${opacity < 0.2 ? '10' : opacity < 0.3 ? '20' : '30'}`}
                                 style={{width: size, height: size}}/>,
                            <div key={i}
                                 className={`rounded-full bg-emerald-300 opacity-${opacity < 0.2 ? '10' : opacity < 0.3 ? '20' : '30'}`}
                                 style={{width: size, height: size}}/>,
                            <div key={i}
                                 className={`rounded-full bg-amber-400 opacity-${opacity < 0.2 ? '10' : opacity < 0.3 ? '20' : '30'}`}
                                 style={{width: size, height: size}}/>,
                            <div key={i}
                                 className={`rounded bg-emerald-600 opacity-${opacity < 0.2 ? '10' : opacity < 0.3 ? '20' : '30'}`}
                                 style={{width: size, height: size}}/>
                        ];
